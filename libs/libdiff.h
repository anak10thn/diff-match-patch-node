...
class diff_match_patch
{
  ...
  */
  ...
  public:
    diff_match_patch() : Diff_Timeout(1.0f),
                         Diff_EditCost(4),
  ...
  private:
    static bool diff_halfMatchI(const string_t &longtext, const string_t &shorttext, int i, HalfMatchResult &best)
    {
        ...
        while ((j = shorttext.find(seed, j + 1)) != string_t::npos)
        {
            ...
            {
                best.mid_common = safeMid(shorttext, j - suffixLength, suffixLength) + safeMid(shorttext, j, prefixLength);
    ...
    static void diff_cleanupSemantic(Diffs &diffs)
    {
        ...
        for (cur_diff = diffs.begin(); cur_diff != diffs.end();)
        {
            ...
            else
            {
                ...
                // sides of it.
                if (!lastequality.empty() && ((int)lastequality.length() <= std::max(length_insertions1, length_deletions1)) && ((int)lastequality.length() <= std::max(length_insertions2, length_deletions2)))
                {
  ...
  public:
    void diff_cleanupEfficiency(Diffs &diffs) const
    {
        ...
        */
                if (!lastequality.empty() && ((pre_ins && pre_del && post_ins && post_del) || (((int)lastequality.length() < Diff_EditCost / 2) && ((pre_ins ? 1 : 0) + (pre_del ? 1 : 0) + (post_ins ? 1 : 0) + (post_del ? 1 : 0)) == 3)))
                {
                    // printf("Splitting: '%s'\n", qPrintable(lastequality));
                    // Walk back to offending equality.
                    // Change second copy to insert.
            ...
            case EQUAL:
                if (count_delete + count_insert > 1)
                {
                    ...
                    if (count_delete != 0 && count_insert != 0)
                    {
                        ...
                        if (commonlength != 0)
                        {
                            (*cur_diff).text = safeMid(text_insert, text_insert.length() - commonlength) + (*cur_diff).text;
            ...
            for (typename Diffs::iterator next_diff = cur_diff; ++next_diff != diffs.end(); prev_diff = cur_diff, cur_diff = next_diff)
            {
                    ...
                    (*next_diff).operation == EQUAL)
                {
                        ...
                        (*cur_diff).text.compare((*cur_diff).text.size() - (*prev_diff).text.size(), (*prev_diff).text.size(), (*prev_diff).text) == 0)
                    {
                        ...
                        (*cur_diff).text = (*prev_diff).text + (*cur_diff).text.substr(0, (*cur_diff).text.length() - (*prev_diff).text.length());
                    ...
                    else if ((*cur_diff).text.size() >= (*next_diff).text.size() && (*cur_diff).text.compare(0, (*next_diff).text.size(), (*next_diff).text) == 0)
                    {
                        ...
                        (*cur_diff).text = safeMid((*cur_diff).text, (*next_diff).text.length()) + (*next_diff).text;
  ...
  public:
    static Diffs diff_fromDelta(const string_t &text1, const string_t &delta)
    {
        ...
        {
            throw string_t(traits::cs(L"Delta length (") + to_string(pointer) + traits::cs(L") smaller than source text length (") + to_string(text1.length()) + traits::from_wchar(L')'));
        }
        return diffs;
    }
  ...
  protected:
    void patch_addContext(Patch &patch, const string_t &text) const
    {
        ...
        while (text.find(pattern) != text.rfind(pattern) && (int)pattern.length() < Match_MaxBits - Patch_Margin - Patch_Margin)
        {
            padding += Patch_Margin;
            pattern = safeMid(text, std::max(0, patch.start2 - padding),
                              std::min((int)text.length(), patch.start2 + patch.length1 + padding) - std::max(0, patch.start2 - padding));
        }
        ...
        string_t suffix = safeMid(text, patch.start2 + patch.length1,
                                  std::min((int)text.length(), patch.start2 + patch.length1 + padding) - (patch.start2 + patch.length1));
        if (!suffix.empty())
        {
        ...
        { // Get rid of the null case.
            ...
            for (typename Diffs::const_iterator cur_diff = diffs.begin(); cur_diff != diffs.end(); ++cur_diff)
            {
                ...
                case INSERT:
                    ...
                    postpatch_text = postpatch_text.substr(0, char_count2) + (*cur_diff).text + safeMid(postpatch_text, char_count2);
                    break;
                case DELETE:
                    patch.length1 += (*cur_diff).text.length();
                    patch.diffs.push_back(*cur_diff);
                    postpatch_text = postpatch_text.substr(0, char_count2) + safeMid(postpatch_text, char_count2 + (*cur_diff).text.length());
                    break;
                case EQUAL:
                    if ((int)(*cur_diff).text.length() <= 2 * Patch_Margin && !patch.diffs.empty() && !(*cur_diff == diffs.back()))
                    {
        ...
        for (typename Patches::const_iterator cur_patch = patchesCopy.begin(); cur_patch != patchesCopy.end(); ++cur_patch)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ((int)text1.length() > Match_MaxBits && diff_levenshtein(diffs) / static_cast<float>(text1.length()) > Patch_DeleteThreshold)
                    {
                    ...
                    else
                    {
                        ...
                        for (typename Diffs::const_iterator cur_diff = (*cur_patch).diffs.begin(); cur_diff != (*cur_patch).diffs.end(); ++cur_diff)
                        {
                            if ((*cur_diff).operation != EQUAL)
                            {
                                ...
                                {
                                    // Deletion
                                    text = text.substr(0, start_loc + index2) + safeMid(text, start_loc + diff_xIndex(diffs,
        ...
        for (typename Patches::iterator cur_patch = patches.begin(); cur_patch != patches.end();)
        {
            ...
            while (!bigpatch.diffs.empty())
            {
                ...
                while (!bigpatch.diffs.empty() && patch.length1 < patch_size - Patch_Margin)
                {
                    ...
                    else if (diff_type == DELETE && patch.diffs.size() == 1 && patch.diffs.front().operation == EQUAL && (int)diff_text.length() > 2 * patch_size)
                    {
        ...
        if (!textline.empty())
        {
            ...
            while (text - textline.c_str() < (int)textline.length())
            {
                ...
                do
                {
                    ...
                    l = text_len;
                    if ((l -= 9) > 0 && traits::to_wchar(*t) == L'@' && traits::to_wchar(*++t) == L'@' && traits::to_wchar(*++t) == L' ' && traits::to_wchar(*++t) == L'-' && traits::is_digit(*++t))
                    {
                        do
                        {
                            start1 += *t;
                        ...
                        if (l > 0 && traits::to_wchar(*t++) == L' ' && traits::to_wchar(*t++) == L'+' && traits::is_digit(*t))
                        {
                            ...
                            while (l > 0 && traits::is_digit(*t))
                                --l, length2 += *t++;
                            if (l == 0 && traits::to_wchar(*t++) == L' ' && traits::to_wchar(*t++) == L'@' && traits::to_wchar(*t) == L'@')
                                break; // Success
                        }
                    }
...
struct diff_match_patch_utf32_from_utf16
{
    ...
    {
        if (u > 0xFFFF)
        { // Encode code points that do not fit in char_t as UTF-16 surrogate pairs
